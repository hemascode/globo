      ...
      <div class="main-content">
        <section class="section">
          ...
          <div class="section-body">
            ...
            <a href="{{ route('admin.shipping-import-page') }}" class="btn btn-success"><i class="fas fa-plus"></i> {{__('Bulk Upload')}}</a>
                                ...
                                @foreach ($shippings as $index => $shipping)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                {{ $shipping->city->name ?? 'None' }}, {{ $shipping->city->countryState->name  ?? 'None'}}, {{ $shipping->city->countryState->country->name ?? 'None' }}
                                            @endif
                                        </td>
                                        <td>
                    ...
                    <div class="container-fluid">
                        <form action="{{ route('admin.shipping.store') }}" method="POST">
                            ...
                            <div class="row">
                                    ...
                                    <select name="city_id" id="" class="form-control select2">
                                        ...
                                        <option value="{{ $city->id }}">
                                            @if($city->name)
                                                {{ $city->name }},
                                            @endif
                                            @if($city->countryState && $city->countryState->name)
                                                {{ $city->countryState->name }},
                                            @endif
                                            @if($city->countryState && $city->countryState->country && $city->countryState->country->name)
                                                {{ $city->countryState->country->name }}
                                            @endif
                                        </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-6">
                                    <label>{{__('admin.Condition To')}} <span class="text-danger">*</span></label>
                                    <div class="input-group mb-3">
                                        <span class="input-group-text type_class"> {{ $setting->currency_icon }}</span>
