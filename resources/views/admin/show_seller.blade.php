...
@section('admin-content')
            ...
            <div class="section-body">
                ...
                <div class="row mt-5">
                    ...
                    <div class="col-md-3">
                        ...
                        @else
                            <!-- Handle the case where $user is null, e.g., redirect or show an error message -->
                        @endif
                    </div>
                    ...
                    <div class="col-12 col-md-12 col-lg-5">
                        <div class="card profile-widget">
                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    <a href="javascript:;"
                                                        onclick="manageCustomerStatus({{ $user ? $user->id : null }})">
                                        ...
                                        <tr>
                                            <td>{{ __('Document Status') }}
                                                @if (isset($seller->files[0]))
                                                    <a href="{{ asset('/' . $seller->files[0]->path) }}" download><i
                                                            class="fas fa-download"></i></a>
                                                @endif
                                            </td>
                                            <td>
                                                @if ($seller && $seller->files[0]->pivot->status->name)
                                                    <div style="cursor: pointer;">
                                                        <select class="form-control"
                                                            id="status_dropdown_{{ $seller->id }}"
                                                            onchange="manageDocumentStatus({{ $seller->id }}, this.value)">
                                                            <option value="1"
                                                                {{ $seller->files[0]->pivot->status->name == 'Pending' ? 'selected' : '' }}>
                                                                {{ __('admin.Pending') }}</option>
                                                            <option value="2"
                                                                {{ $seller->files[0]->pivot->status->name == 'Verified' ? 'selected' : '' }}>
                                                                {{ __('admin.Verified') }}</option>
                                                            <option value="3"
                                                                {{ $seller->files[0]->pivot->status->name == 'Rejected' ? 'selected' : '' }}>
                            ...
                            <div class="card-footer text-center">
                                <div class="font-weight-bold mb-2">{{ __('admin.Follow') }}
                                    {{ $user ? $user->name : '-' }}</div>
                    ...
                    <div class="col-12 col-md-12 col-lg-7">
                        <div class="card">
                            <form method="post" class="needs-validation" novalidate=""
                                action="{{ $user ? route('admin.seller-update', $user->id) : '#' }}">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="form-group col-6">
                                            ...
                                            <input type="email" class="form-control"
                                                value="{{ $user ? $user->email : '' }}" name="email" readonly>
                                        ...
                                        <div class="form-group col-6">
                                            ...
                                            <input type="text" class="form-control"
                                                value="{{ $user ? $user->phone : '' }}" name="phone">
                                        ...
                                        <div class="form-group col-6">
                                            ...
                                            <input type="text" class="form-control"
                                                value="{{ $user ? $user->address : '' }}" name="address">
                                        </div>
    ...
    <script>
        ...
        function manageDocumentStatus(id, status) {
            // Example AJAX call using jQuery
            $.ajax({
                url: "{{ url('/admin/seller-document-status/') }}" + "/" + id,
                type: 'POST',
                data: {
                    seller_id: id,
                    status: status,
