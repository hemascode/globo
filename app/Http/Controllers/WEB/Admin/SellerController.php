...
class SellerController extends Controller
{
    ...
    {
        $seller = Vendor::with('user', 'socialLinks', 'products', 'files')->find($id);
        // dd($seller->files[0]->path);
        if ($seller) {
            ...
            if ($user) {
                if ($user->country_id) {
                    $states = CountryState::with('cities', 'country')
                        ->orderBy('name', 'asc')
                        ->where(['status' => 1, 'country_id' => $user->country_id])
                        ->get();
                }

                if ($user->state_id) {
                    $cities = City::with('countryState')
                        ->orderBy('name', 'asc')
                        ->where(['status' => 1, 'country_state_id' => $user->state_id])
                        ->get();
                }
            }
            ...
            foreach ($orderProducts as $orderProduct) {
                if ($orderProduct->order && $orderProduct->order->payment_status == 1 && $orderProduct->order->order_status == 3) {
                    $price = ($orderProduct->unit_price * $orderProduct->qty) + $orderProduct->vat;
                    $totalAmount += $price;
                    $totalSoldProduct += $orderProduct->qty;
                }
            }
    ...
    public function changeDocumentStatus(Request $request)
    {
        ...
        $seller = Vendor::find($request->seller_id);
        $seller_verfication = VendorVerification::where('vendor_id', $request->seller_id)->first();

        // Assuming you have a relation named 'files' and you want to update the first file's status
        // $file = $seller->files->first();
        // dd($file);
        if ($seller_verfication) {
            $seller_verfication->status_id = $request->status;

            $seller_verfication->save();
            return response()->json(['message' => 'Status updated successfully.']);
        }

        return response()->json(['error' => 'File not found for the specified seller.'], 404);
    }
