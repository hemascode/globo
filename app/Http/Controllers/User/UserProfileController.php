...
class UserProfileController extends Controller
{
    ...
    {
        $user = Auth::guard('api')->user();
        $saveforlater = SaveForLater::with('product')->where(['user_id' => $user->id])->paginate(10);

        return response()->json(['saveforlater' => $saveforlater]);
    }
    ...
    public function addToSaveForLater($id)
    {
        $user = Auth::guard('api')->user();
        $product = Product::find($id);
        $isExist = SaveForLater::where(['user_id' => $user->id, 'product_id' => $product->id])->count();
        if ($isExist == 0) {
            $saveforlater = new SaveForLater();
            $saveforlater->product_id = $id;
            $saveforlater->user_id = $user->id;
            $saveforlater->save();
            $message = trans('user_validation.Product Saved');
            return response()->json(['message' => $message]);
        } else {
            $message = trans('user_validation.Product Already saved');
            return response()->json(['message' => $message], 403);
        }
    }
    ...
    {
        $user = Auth::guard('api')->user();
        SaveForLater::where(['user_id' => $user->id])->delete();
