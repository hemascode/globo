...
class OrderController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $order = Order::with('user', 'orderProducts.orderProductVariants', 'orderAddress')->find($request->id);
    ...
    public function destroy(Request $request)
    {
        $order = Order::find($request->id);
        if (!$order) {
            // Order not found, return an error message
            return response()->json(['message' => 'Order not found'], 404);
        }
        $order->delete();
        $orderProducts = OrderProduct::where('order_id', $request->id)->get();
        $orderAddress = OrderAddress::where('order_id', $request->id)->first();
        foreach ($orderProducts as $orderProduct) {
            OrderProductVariant::where('order_product_id', $orderProduct->id)->delete();
            $orderProduct->delete();
        }
        OrderAddress::where('order_id', $request->id)->delete();
